...
static void asserted(const int cond, const char* const file_path, const int line, const char* const expression) {
    ...
    if (cond) {
        ...
        printf("[%s][YASER - " ANSI_COLOR_YELLOW "ASSERTION" ANSI_COLOR_RESET "][%s:%d]:" ANSI_COLOR_YELLOW " %s does not hold\n" ANSI_COLOR_RESET, time_buffer, basename, line, expression);
        cleanup_all();
        exit(EXIT_FAILURE);
    }
#else
